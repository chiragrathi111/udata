...
WITH RECURSIVE cte AS (
    SELECT 
        ...
        cl.c_uuid,
        cl.created,
        var.name AS variety,cl.parentcultureline AS parentcultureline,
        NULL AS village,
        NULL AS talukname,
        NULL AS district,NULL AS collection_updated_date
    FROM adempiere.tc_culturelabel cl
    JOIN adempiere.tc_variety var ON var.tc_variety_id = cl.tc_variety_id
    WHERE TRIM(cl.c_uuid) = TRIM('8a6decb9-dfa7-496d-8b23-21bc889f2d8b')
    ...
    JOIN adempiere.tc_culturelabel cl ON phs.cultureuuid = cl.c_uuid
    JOIN adempiere.tc_variety var ON var.tc_variety_id = cl.tc_variety_id
    WHERE TRIM(ph.c_uuid) = TRIM('8a6decb9-dfa7-496d-8b23-21bc889f2d8b')
    ...
    JOIN adempiere.tc_culturelabel cl ON phs.cultureuuid = cl.c_uuid
    JOIN adempiere.tc_variety var ON var.tc_variety_id = cl.tc_variety_id
    WHERE TRIM(sh.c_uuid) = TRIM('8a6decb9-dfa7-496d-8b23-21bc889f2d8b')
    ...
    SELECT 
        ...
        cl2.c_uuid,
        cl2.created,
        var.name AS variety,cl2.parentcultureline AS parentcultureline,
        NULL AS village,
        NULL AS talukname,
        NULL AS district,NULL AS collection_updated_date
    FROM cte 
...
culture_result AS (
    SELECT 
        ...
        cte.c_uuid,
        cte.created,
        cte.variety,cte.parentcultureline,
        cte.village,
        cte.talukname,
        cte.district,cte.collection_updated_date
    FROM cte
    GROUP BY cte.parentuuid, cte.c_uuid, cte.created, cte.variety,cte.parentcultureline, cte.village, cte.talukname, cte.district,cte.collection_updated_date
    ...
    SELECT DISTINCT 
        ...
        tcc.c_uuid,
        tcc.created,
        cte.variety,tcc.parentcultureline,
        cte.village,
        cte.talukname,
        cte.district,NULL AS collection_updated_date
    FROM cte 
    ...
    SELECT DISTINCT 
        ...
        tpt.c_uuid,
        tpt.created,
        cte.variety,pd.parentcultureline,
        f.villagename2 AS village,
        f.talukname,
        f.district,TO_CHAR(cp.updated, 'DD/MM/YYYY') AS collection_updated_date
    FROM cte 
...
explant_result AS (
    SELECT DISTINCT 
        ...
        tcc.c_uuid,
        tcc.created,
        var.name AS variety,tcc.parentcultureline,
        NULL AS village,
        NULL AS talukname,
        NULL AS district,NULL AS collection_updated_date
    FROM adempiere.tc_explantlabel tcc 
    JOIN adempiere.tc_variety var ON var.tc_variety_id = tcc.tc_variety_id
    WHERE TRIM(tcc.c_uuid) = TRIM('8a6decb9-dfa7-496d-8b23-21bc889f2d8b')
    ...
    SELECT DISTINCT 
        ...
        tpt.c_uuid,
        tpt.created,
        var.name AS variety,pd.parentcultureline,
        f.villagename2 AS village,
        f.talukname,
        f.district,TO_CHAR(cp.updated, 'DD/MM/YYYY') AS collection_updated_date
    FROM adempiere.tc_planttag tpt 
...
plant_tag_result AS (
    ...
    JOIN adempiere.tc_farmer f ON pd.tc_farmer_id = f.tc_farmer_id
    JOIN adempiere.tc_variety var ON var.tc_variety_id = pd.tc_variety_id
    WHERE TRIM(tpt.c_uuid) = TRIM('8a6decb9-dfa7-496d-8b23-21bc889f2d8b')
),
...
primary_result AS (
    SELECT 
        ...
        ph.c_uuid,
        ph.created,
        var.name AS variety,ph.parentcultureline,
        NULL AS village,
        NULL AS talukname,
        NULL AS district,NULL AS collection_updated_date
    FROM adempiere.TC_PrimaryHardeningLabel ph 
...
secondary_result AS (
    SELECT 
        ...
        sh.c_uuid,
        sh.created,
        var.name AS variety,sh.parentcultureline,
        NULL AS village,
        NULL AS talukname,
        NULL AS district,NULL AS collection_updated_date
    FROM adempiere.TC_SecondaryHardeningLabel sh 
...
q2.parentcultureline AS parentcultureline,
q2.variety AS variety,
...
cr.cursymbol,ENCODE(org_img.binarydata,'base64') AS logo_binarydata,cli.Name AS companyname,corl.cultureuuid
...
LEFT JOIN adempiere.tc_variety tv_ph ON tv_ph.tc_variety_id = ph.tc_variety_id
LEFT JOIN adempiere.c_orderline corl ON ivl.c_orderline_id = corl.c_orderline_id
LEFT JOIN LATERAL (
WITH RECURSIVE cte AS (
SELECT cl.parentuuid,cl.c_uuid,cl.created,var.name AS variety,cl.parentcultureline AS parentcultureline,NULL AS village,NULL AS talukname,NULL AS district,NULL AS collection_updated_date
FROM adempiere.tc_culturelabel cl JOIN adempiere.tc_variety var ON var.tc_variety_id = cl.tc_variety_id
WHERE cl.c_uuid = COALESCE(cor.secondaryhardeninguuid, corl.cultureuuid)
UNION ALL
SELECT phs.cultureuuid AS parentuuid,cl.c_uuid,cl.created,var.name AS variety,cl.parentcultureline AS parentcultureline,NULL AS village,NULL AS talukname,NULL AS district,
NULL AS collection_updated_date FROM adempiere.TC_PrimaryHardeningLabel ph
JOIN adempiere.tc_primaryHardeningcultureS phs ON phs.TC_PrimaryHardeningLabel_id = ph.TC_PrimaryHardeningLabel_id
JOIN adempiere.tc_culturelabel cl ON phs.cultureuuid = cl.c_uuid JOIN adempiere.tc_variety var ON var.tc_variety_id = cl.tc_variety_id
WHERE ph.c_uuid = COALESCE(cor.secondaryhardeninguuid, corl.cultureuuid)
UNION ALL
SELECT phs.cultureuuid AS parentuuid,cl.c_uuid,cl.created,var.name AS variety,cl.parentcultureline AS parentcultureline,NULL AS village,NULL AS talukname,NULL AS district,
NULL AS collection_updated_date FROM adempiere.TC_SecondaryHardeningLabel sh JOIN adempiere.TC_PrimaryHardeningLabel ph ON sh.parentuuid = ph.c_uuid
JOIN adempiere.tc_primaryHardeningcultureS phs ON phs.TC_PrimaryHardeningLabel_id = ph.TC_PrimaryHardeningLabel_id
JOIN adempiere.tc_culturelabel cl ON phs.cultureuuid = cl.c_uuid JOIN adempiere.tc_variety var ON var.tc_variety_id = cl.tc_variety_id
WHERE sh.c_uuid = COALESCE(cor.secondaryhardeninguuid, corl.cultureuuid)
UNION ALL
SELECT cl2.parentuuid,cl2.c_uuid,cl2.created,var.name AS variety,cl2.parentcultureline AS parentcultureline,NULL AS village,NULL AS talukname,NULL AS district,
NULL AS collection_updated_date FROM cte JOIN adempiere.tc_culturelabel cl2 ON cte.parentuuid = cl2.c_uuid
JOIN adempiere.tc_variety var ON var.tc_variety_id = cl2.tc_variety_id),
culture_result AS (
SELECT cte.parentuuid,cte.c_uuid,cte.created,cte.variety,cte.parentcultureline,cte.village,cte.talukname,cte.district,cte.collection_updated_date
FROM cte GROUP BY cte.parentuuid, cte.c_uuid, cte.created, cte.variety,cte.parentcultureline, cte.village, cte.talukname, cte.district, cte.collection_updated_date
UNION ALL
SELECT DISTINCT tcc.parentuuid,tcc.c_uuid,tcc.created,cte.variety,cte.parentcultureline,cte.village,cte.talukname,cte.district,NULL AS collection_updated_date
FROM cte LEFT JOIN adempiere.tc_explantlabel tcc ON cte.parentuuid = tcc.c_uuid
UNION ALL
SELECT DISTINCT NULL,tpt.c_uuid,tpt.created,cte.variety,cte.parentcultureline,f.villagename2 AS village,f.talukname,f.district,
TO_CHAR(cp.updated, 'DD/MM/YYYY') AS collection_updated_date FROM cte
LEFT JOIN adempiere.tc_explantlabel tcc ON cte.parentuuid = tcc.c_uuid LEFT JOIN adempiere.tc_planttag tpt ON tcc.parentuuid = tpt.c_uuid
JOIN adempiere.tc_plantdetails pd ON pd.planttaguuid = tpt.c_uuid
JOIN adempiere.TC_collectionjoinplant cp ON cp.TC_PlantDetails_ID = pd.TC_PlantDetails_ID
JOIN adempiere.tc_farmer f ON pd.tc_farmer_id = f.tc_farmer_id WHERE tpt.c_uuid IS NOT NULL),
explant_result AS (
SELECT DISTINCT tcc.parentuuid,tcc.c_uuid,tcc.created,var.name AS variety,tcc.parentcultureline,NULL AS village,NULL AS talukname,NULL AS district,
NULL AS collection_updated_date FROM adempiere.tc_explantlabel tcc
JOIN adempiere.tc_variety var ON var.tc_variety_id = tcc.tc_variety_id WHERE tcc.c_uuid = COALESCE(cor.secondaryhardeninguuid, corl.cultureuuid)
UNION ALL
SELECT DISTINCT NULL,tpt.c_uuid,tpt.created,var.name AS variety,pd.parentcultureline,f.villagename2 AS village,f.talukname,f.district,
TO_CHAR(cp.updated, 'DD/MM/YYYY') AS collection_updated_date FROM adempiere.tc_planttag tpt
JOIN adempiere.tc_explantlabel tcc ON tcc.parentuuid = tpt.c_uuid JOIN adempiere.tc_plantdetails pd ON pd.planttaguuid = tpt.c_uuid
JOIN adempiere.TC_collectionjoinplant cp ON cp.TC_PlantDetails_ID = pd.TC_PlantDetails_ID JOIN adempiere.tc_farmer f ON pd.tc_farmer_id = f.tc_farmer_id
JOIN adempiere.tc_variety var ON var.tc_variety_id = pd.tc_variety_id
WHERE tcc.c_uuid = COALESCE(cor.secondaryhardeninguuid, corl.cultureuuid) AND tpt.c_uuid IS NOT NULL),
plant_tag_result AS (
SELECT DISTINCT NULL,tpt.c_uuid,tpt.created,var.name AS variety,pd.parentcultureline,f.villagename2 AS village,f.talukname,f.district,
TO_CHAR(cp.updated, 'DD/MM/YYYY') AS collection_updated_date FROM adempiere.tc_planttag tpt
JOIN adempiere.tc_plantdetails pd ON pd.planttaguuid = tpt.c_uuid JOIN adempiere.TC_collectionjoinplant cp ON cp.TC_PlantDetails_ID = pd.TC_PlantDetails_ID
JOIN adempiere.tc_farmer f ON pd.tc_farmer_id = f.tc_farmer_id JOIN adempiere.tc_variety var ON var.tc_variety_id = pd.tc_variety_id
WHERE tpt.c_uuid = COALESCE(cor.secondaryhardeninguuid, corl.cultureuuid)),
primary_result AS (
SELECT phs.cultureuuid AS parentuuid,ph.c_uuid,ph.created,var.name AS variety,ph.parentcultureline,NULL AS village,NULL AS talukname,NULL AS district,
NULL AS collection_updated_date FROM adempiere.TC_PrimaryHardeningLabel ph
JOIN adempiere.tc_primaryHardeningcultureS phs ON phs.TC_PrimaryHardeningLabel_id = ph.TC_PrimaryHardeningLabel_id
JOIN adempiere.tc_variety var ON var.tc_variety_id = ph.tc_variety_id WHERE ph.c_uuid = COALESCE(cor.secondaryhardeninguuid, corl.cultureuuid)),
secondary_result AS (
SELECT sh.parentuuid,sh.c_uuid,sh.created,var.name AS variety,sh.parentcultureline,NULL AS village,NULL AS talukname,NULL AS district,NULL AS collection_updated_date
FROM adempiere.TC_SecondaryHardeningLabel sh JOIN adempiere.tc_variety var ON var.tc_variety_id = sh.tc_variety_id
WHERE sh.c_uuid = COALESCE(cor.secondaryhardeninguuid, corl.cultureuuid)
UNION ALL
SELECT phs.cultureuuid AS parentuuid,ph.c_uuid,ph.created,var.name AS variety,ph.parentcultureline,NULL AS village,NULL AS talukname,NULL AS district,
...
JOIN adempiere.TC_PrimaryHardeningLabel ph ON sh.parentuuid = ph.c_uuid
JOIN adempiere.tc_primaryHardeningcultureS phs ON phs.TC_PrimaryHardeningLabel_id = ph.TC_PrimaryHardeningLabel_id
JOIN adempiere.tc_variety var ON var.tc_variety_id = ph.tc_variety_id WHERE sh.c_uuid = COALESCE(cor.secondaryhardeninguuid, corl.cultureuuid))
SELECT * FROM secondary_result  
UNION ALL
SELECT * FROM primary_result
UNION ALL
...
WHERE (iv.C_Invoice_ID = 1000059 ) AND iv.ad_client_id = 1000002 ORDER BY ivl.line;
-------------------------------------------------------------------------------------------------------------------