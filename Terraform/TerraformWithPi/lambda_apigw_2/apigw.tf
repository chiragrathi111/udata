...
# API Gateway Resource for User Details - Creates /users endpoint
# This creates the /users endpoint for POST requests with user data
resource "aws_api_gateway_resource" "users_resource" {
  rest_api_id = aws_api_gateway_rest_api.lambda_api.id
  parent_id   = aws_api_gateway_rest_api.lambda_api.root_resource_id
  path_part   = "users"  # This creates /users endpoint
}
...
# API Gateway Method for POST /users - Accepts user data in request body
# This allows POST requests to /users endpoint with JSON body
resource "aws_api_gateway_method" "users_post" {
  rest_api_id   = aws_api_gateway_rest_api.lambda_api.id
  resource_id   = aws_api_gateway_resource.users_resource.id
  http_method   = "POST"                   # HTTP POST method
  authorization = "NONE"                   # No authentication required
  
  # Request validation - ensures request has proper JSON body
  request_validator_id = aws_api_gateway_request_validator.users_validator.id
  
  # Request models - defines expected JSON structure
  request_models = {
    "application/json" = aws_api_gateway_model.user_model.name
  }
}

# API Gateway Request Validator - Validates incoming requests
# This ensures POST requests have valid JSON body with required fields
resource "aws_api_gateway_request_validator" "users_validator" {
  name                        = "users-validator"
  rest_api_id                = aws_api_gateway_rest_api.lambda_api.id
  validate_request_body      = true   # Validate JSON body
  validate_request_parameters = false # Don't validate query params
}

# API Gateway Model - Defines expected JSON structure for user data
# This defines the schema for user data: {"name": "string", "age": number}
...
# API Gateway Integration for POST /users - Connects POST endpoint to Lambda
# This tells API Gateway how to call Lambda function for user data processing
resource "aws_api_gateway_integration" "users_lambda_integration" {
  ...
  integration_http_method = "POST"                                    # Lambda always uses POST
  type                   = "AWS_PROXY"                               # Lambda proxy integration
  uri                    = aws_lambda_function.lambda_apigw.invoke_arn # Same Lambda function ARN
}
...
# API Gateway Method Response for POST /users - Defines response for user creation
# This tells API Gateway what responses to expect from Lambda for user data
resource "aws_api_gateway_method_response" "users_response_200" {
  rest_api_id = aws_api_gateway_rest_api.lambda_api.id
  resource_id = aws_api_gateway_resource.users_resource.id
  http_method = aws_api_gateway_method.users_post.http_method
  status_code = "200"  # HTTP 200 OK response
...
# API Gateway Method Response for POST /users - Error response
# This handles validation errors and bad requests
resource "aws_api_gateway_method_response" "users_response_400" {
  rest_api_id = aws_api_gateway_rest_api.lambda_api.id
  resource_id = aws_api_gateway_resource.users_resource.id
  http_method = aws_api_gateway_method.users_post.http_method
  status_code = "400"  # HTTP 400 Bad Request response
...
# API Gateway Integration Response for POST /users - Maps Lambda response to API response
# This tells API Gateway how to handle Lambda function responses for user data
resource "aws_api_gateway_integration_response" "users_integration_response" {
  rest_api_id = aws_api_gateway_rest_api.lambda_api.id
  resource_id = aws_api_gateway_resource.users_resource.id
  http_method = aws_api_gateway_method.users_post.http_method
  status_code = aws_api_gateway_method_response.users_response_200.status_code
  ...
  depends_on = [aws_api_gateway_integration.users_lambda_integration]
}
